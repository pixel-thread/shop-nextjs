...
import { pageAccessOnlyIfUnAuthenticated, routeRoles } from "./routeRole";
import { AUTH_TOKEN_KEY } from "@/lib/constants/token";

type PropsT = Readonly<{ children: React.ReactNode }>;
...
export const RoleBaseRoute = ({ children }: PropsT) => {
  ...
  const [cookies] = useCookies([AUTH_TOKEN_KEY]); // Get the AUTH_TOKEN cookie
  const userRoles: string = useMemo(() => user?.role || "USER", [user]); // Get the user's roles
  const isAuthenticated = !!(user && cookies.AuthToken); //
    ...
    if (currentRoute) {
      ...
      if (isAuthenticated) {
        ...
        if (!hasRequiredRole) {
          ...
          router.replace(currentRoute.redirect || "/error");
          return; // Exit the logic as redirection is in progress
        }
      }
    }
