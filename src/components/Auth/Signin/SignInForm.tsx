...
type FormValuesI = z.infer<typeof authSchema>;
...
export const SignInForm = () => {
  ...
  const { mutate: sendOtp, isPending: isLoadingInit } = useMutation({
    mutationFn: async (data: FormValuesI) =>
      http.post(AUTH_ENDPOINT.POST_LOGIN_INIT, data),
    onSuccess: (data) => {
      if (data.success) {
        setIsOtpStage(true);
  ...
  const { mutate: onLogin, isPending: isLogging } = useMutation({
    mutationFn: async (data: FormValuesI) =>
      http.post(AUTH_ENDPOINT.POST_LOGIN, data),
    onSuccess: (data) => {
      if (data.success) {
        toast({ title: data.message });
        setIsOtpStage(false);
  ...
  const isLoading = isAuthLoading || isLogging || isLoadingInit;
    ...
    <div>
      <Form {...form}>
        <form noValidate onSubmit={form.handleSubmit(onSubmit)}>
          ...
          >
            {isOtpStage ? "Verify OTP" : "Send OTP"}
          </button>
