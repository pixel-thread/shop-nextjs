...
import { PRODUCT_ENDPOINT } from "@/lib/constants/endpoints/product";
import PreLoader from "../Common/PreLoader";
import { Ternary } from "../Common/Ternary";
import { SingleGridItemSkeleton } from "../Shop/SingleGridItemSkeleton";
...
const ShopWithSidebar = () => {
  ...
  const { isFetching, isPending, data, isLoading } = useQuery({
    queryKey: ["products", page],
    queryFn: () =>
      http.get<Product[]>(PRODUCT_ENDPOINT.GET_PRODUCT.replace(":page", page)),
  });
                  ...
                  ifTrue={
                    <>
                      {Array.from({ length: 20 }).map((_, i) => (
                        <SingleGridItemSkeleton key={i} />
                      ))}
                    </>
                  }
                  ifFalse={
                    <Ternary
                      condition={product.length === 0}
                      ifTrue={
                        <>
                          <div className="flex items-center justify-center">
                            <div className="flex items-center w-full gap-2">
                              <p className="font-semibold text-center text-dark text-lg">
                                No products found
                              </p>
                            </div>
                          </div>
